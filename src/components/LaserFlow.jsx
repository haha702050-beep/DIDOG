...
export default function LaserFlow({
  ...
  useEffect(() => {
    ...
    baseDprRef.current = Math.min(dpr ?? window.devicePixelRatio ?? 1, 1.6);
    ...
    const adjustDprIfNeeded = (now) => {
      ...
      if (avgFps > upperThresh && currentDprRef.current < base) next = clamp(currentDprRef.current * 1.1, dprFloor, base);
